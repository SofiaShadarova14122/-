...
def btn_finish_click():
    ...
    header.grid_remove()
    result_test = Label(text=f'Вы дали {count_true_answers} правильных ответов из {len(qas)}', padx=15, pady=10)
    result_test.grid(row=0, column=0, sticky=W)
    messagebox.showinfo("Завершение теста","Это конец теста;)")
root = Tk()
qas = list()
cur_qa = -1
...
radio_buttons = list()
btn_ok = Button(text="Далее", background="#53377a", foreground="#d5cade",font="16", command=btn_ok_click)
btn_finish = Button(text="Завершить", background="#53377a", foreground="#d5cade",font="16", command=btn_finish_click)
...
def main():
    ...
    root.title("Маленький тест по сериалу Доктор Кто")
