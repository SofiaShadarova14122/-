...
try:
    ...
    def print_result(perevorot, korotky, start, finish):
        path = []
        znak = finish
        if znak == start:
        ...
        while znak != start:
            ...
            znak = perevorot[znak]
        path.append(start)
        print("Кратчайшая дорога: {}".format(korotky[finish]))
    ...
    start=input('Отправная точка(заглавные латинские буквы): ')
    finish=input('Точка прибытия(заглавные латинские буквы): ')
    perevorot, korotky = issledovatel(graph=graph, start=start)

    print_result(perevorot, korotky, start=start, finish=finish)
except ValueError:
    print('Числа графа:')
