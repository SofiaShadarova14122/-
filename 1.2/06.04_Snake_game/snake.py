...
pygame.display.set_caption('Змейка из Магнолии))')
...
def gameLoop():
    ...
    while not game_over:
        if game_close == True:
            s_gameover = pygame.mixer.Sound("gameover.mp3")
            s_gameover.play()
        while game_close == True:
            message("Конец игры:(", white)
            message2("Exit - Esc Continue - Space", white)
        ...
        if x1 == foodx and y1 == foody:
            ...
            foody = round(random.randrange(100, height - snake_block) / 10.0) * 10.0
            Length_of_snake += 1
            s_catch = pygame.mixer.Sound("lut.mp3")
            s_catch.play()
